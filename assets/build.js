...
require.register("uhave-breadtrip/app/controller/select-photos.js", function(exports, require, module){
var j = require('jquery');
var controller = require('controller');

module.exports = function () {
  new SelectPhotos(document.documentElement);
};

var SelectPhotos = controller(function (el) {
  var self = this;

  self.el = el;
  self.journeyLists = [];

  var selectAllJourneysElement = self.select('.select-all-journeys');

  selectAllJourneysElement.onclick = function (clickEvent) {
    if (j(selectAllJourneysElement).hasClass('selected')) {
      self.journeyLists.forEach(function (journeyList, i) {
        journeyList.deselectAllJourneys();
      });

      j(selectAllJourneysElement).removeClass('selected');
    } else {
      self.journeyLists.forEach(function (journeyList, i) {
        journeyList.selectAllJourneys();
      });

      j(selectAllJourneysElement).addClass('selected');
    }
  }

  self.selectAll('.block.journey-list').forEach(function (el, index) {
    var journeyList = new JourneyList(el);

    self.journeyLists.push(journeyList);

    journeyList.on('change', function () {
      /**
       * Count how many journeys selected
       */
      var totalJourneysCount = 0;

      self.journeyLists.forEach(function (journeyList, i) {
        var selectedJourneyTexts = journeyList.getSelectedJourneyTexts();
        var selectedJourneyPhotos = journeyList.getSelectedJourneyPhotos();

        totalJourneysCount += selectedJourneyTexts.length + selectedJourneyPhotos.length;
      });

      self.select('.block.money-belt .total-count').textContent = totalJourneysCount;

      if (totalJourneysCount >= 0) {
        self.select('.block.money-belt .checkout-amount').textContent = '100';
      }

      if (totalJourneysCount >= 3) {
        self.select('.block.money-belt .checkout-amount').textContent = '300';
      }

      if (totalJourneysCount >= 6) {
        self.select('.block.money-belt .checkout-amount').textContent = '600';
      }

      /**
       * Select all state
       */
      for (var i in self.journeyLists) {
        if (self.journeyLists[i].allSelected === false) {
          j(selectAllJourneysElement).removeClass('selected');
          return;
        }
      }
...
var JourneyList = controller(function (el) {
  var self = this;
  ...
  self.journeyDay = new JourneyDay(self.select('.journey.journey-day'));

  self.journeyDay.on('user-selected', function () {
    self.journeyPhotos.forEach(function (journeyPhoto, index) {
      journeyPhoto.select();
    });

    self.journeyTexts.forEach(function (journeyText, index) {
      journeyText.select();
    });
  });

  self.journeyDay.on('user-deselected', function () {
    self.journeyPhotos.forEach(function (journeyPhoto, index) {
      journeyPhoto.deselect();
    });

    self.journeyTexts.forEach(function (journeyText, index) {
      journeyText.deselect();
    });
  });

  self.selectAll('.journey.journey-text').forEach(function (el, index) {
    var journeyText = new JourneyText(el);

    self.journeyTexts.push(journeyText);

    journeyText.on('deselect', function () {
      self.journeyDay.deselect();
      self.allSelected = false;
      self.trigger('change');
    });

    journeyText.on('select', function () {
      for (var i in self.journeyTexts) {
        if (self.journeyTexts[i].selected === false) {
          self.allSelected = false;
          self.trigger('change');
          return;
        }
      ...
      for (var i in self.journeyPhotos) {
        if (self.journeyPhotos[i].selected === false) {
          self.allSelected = false;
          self.trigger('change');
          return;
        }
      ...
      self.journeyDay.select();
      self.allSelected = true;
      self.trigger('change');
    });
  });

  self.selectAll('.journey.journey-photo').forEach(function (el, index) {
    var journeyPhoto = new JourneyPhoto(el);

    self.journeyPhotos.push(journeyPhoto);

    journeyPhoto.on('deselect', function () {
      self.journeyDay.deselect();
      self.allSelected = false;
      self.trigger('change');
    });

    journeyPhoto.on('select', function () {
      for (var i in self.journeyTexts) {
        if (self.journeyTexts[i].selected === false) {
          self.allSelected = false;
          self.trigger('change');
...
JourneyList.prototype.allSelected = false;

JourneyList.prototype.getSelectedJourneyTexts = function () {
  return this.journeyTexts.filter(function (journeyText, index) {
    if (journeyText.selected) {
      return journeyText;
    }
  });
}

JourneyList.prototype.getSelectedJourneyPhotos = function () {
  return this.journeyPhotos.filter(function (journeyPhoto, index) {
    if (journeyPhoto.selected) {
      return journeyPhoto;
    }
  });
}

JourneyList.prototype.selectAllJourneys = function () {
  this.journeyDay.select();

  this.journeyTexts.forEach(function (journeyText, i) {
    journeyText.select();
  });

  this.journeyPhotos.forEach(function (journeyPhoto, i) {
    journeyPhoto.select();
  });
}

JourneyList.prototype.deselectAllJourneys = function () {
  this.journeyDay.deselect();
...
var JourneyDay = controller(function (el) {
  var self = this;

  self.el = el;

  self.el.onclick = function (clickEvent) {
    if (j(self.el).hasClass('selected')) {
      self.deselect();
      self.trigger('user-deselected');
    } else {
      self.select();
      self.trigger('user-selected');
    }
  }
});

JourneyDay.prototype.selected = false;

JourneyDay.prototype.select = function () {
  this.selected = true;
  j(this.el).addClass('selected');
  this.trigger('select');
}

JourneyDay.prototype.deselect = function () {
  this.selected = false;
  j(this.el).removeClass('selected');
  this.trigger('deselect');
}

var JourneyPhoto = controller(function (el) {
  var self = this;
...
JourneyPhoto.prototype.selected = false;

JourneyPhoto.prototype.select = function () {
  this.selected = true;
  j(this.el).addClass('selected');
  this.trigger('select');
}

JourneyPhoto.prototype.deselect = function () {
  this.selected = false;
  j(this.el).removeClass('selected');
  this.trigger('deselect');
}

var JourneyText = controller(function (el) {
  var self = this;
...
JourneyText.prototype.selected = false;

JourneyText.prototype.select = function () {
  this.selected = true;
  j(this.el).addClass('selected');
  this.trigger('select');
}

JourneyText.prototype.deselect = function () {
  this.selected = false;
  j(this.el).removeClass('selected');
  this.trigger('deselect');
}
